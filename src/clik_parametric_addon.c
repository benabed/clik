...
SmicaComp * clik_smica_comp_radiogal_init(hid_t comp_id, char* cur_lkl,int nb, int m, int nell, int* ell, int* has_cl, double unit,double* wl, double *bins, int nbins,error **err) {
  parametric* p_model;
  SmicaComp *SC;
  int lmin,lmax;
  int *detlist;
  ...
  p_model = radiogal_init(m, detlist, ndef, defkeys, defvalues, nvar, varkeys, lmin, lmax, err);
  forwardError(*err,__LINE__,NULL);
