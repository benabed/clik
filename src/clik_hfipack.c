...
cmblkl* clik_smica_init(hid_t group_id, char* cur_lkl, int nell, int* ell, int* has_cl, double unit,double* wl, double *bins, int nbins, error **err) {
  ...
  for(ic=1;ic<nc;ic++) {
    ...
    if (strcasecmp(comp_type, "1d")==0 || strcasecmp(comp_type, "nd")==0) {
      double *someA;
      int nA;
      // try to read A
      ...
      if (hstat==1) {
        nA = -1;
        someA = hdf5_double_attarray(group_id,cur_lkl,"A_cmb",&nA,err);
      }
      if (strcasecmp(comp_type, "1d")==0) {
        testErrorRetVA(someA!=NULL && nA!=m,-100,"Not enough data in %s/%s (expected %d got %d)",*err,__LINE__,NULL,cur_lkl,cur_cmp,ndum,m);
        SCs[ic] = comp_1D_init(nb, m, someA, err);
        forwardError(*err,__LINE__,NULL);    
      } else {
        int nd;
        
        hstat = H5LTget_attribute_int(comp_id, ".", "nd",  &nd);
        testErrorRetVA(hstat<0,hdf5_base,"cannot read nd in component %s/%s (got %d)",*err,__LINE__,NULL,cur_lkl,cur_cmp,hstat);
        
        testErrorRetVA(someA!=NULL && nA!=m*nd,-100,"Not enough data in %s/%s (expected %d got %d)",*err,__LINE__,NULL,cur_lkl,cur_cmp,ndum,m*nd);
        SCs[ic] = comp_nD_init(nb, m, nd, someA, err);
        forwardError(*err,__LINE__,NULL);    
      }
      if (someA!=NULL) {
        free(someA);
    ...
    if (strcasecmp(comp_type, "diag")==0) {
      SCs[ic] = comp_diag_init(nb, m, err);
      forwardError(*err,__LINE__,NULL);    
    }
