    ...
    public like_init,calc_like,CAMSpec_like_version, camspec_beam_mcmc_num, &
        ...
        compute_fg, Nspec,CAMSpec_lmax_foreground,camspec_fiducial_foregrounds,camspec_fiducial_cl,lminX,lmaxX
    contains
...
100     if (j/=i) stop 'error reading fiducial C_l for beams'
    ...
    if (marge_num>0) then
        ...
        call Matrix_Inverse_internal(beam_cov)
    ...
    if (pre_marged) then
        ...
        if (marge_num>0) then
            ...
            call Matrix_inverse_internal(c_inv) !now c_inv is the inverse covariance
            ...
            call Matrix_inverse_internal(c_inv)
        end if
    end if !pre-marged
