...
double slik_lkl(void* pbic, double* pars, error **err) {
  double lkl;
  slik *bic;
  char cmd[500];
  bic = pbic;
...
cmblkl* clik_slik_init(cldf *df, int nell, int* ell, int* has_cl, double unit,double* wl, double *bins, int nbins, error **err) {
  ...
  bic = malloc_err(sizeof(slik),err);
  forwardError(*err,__LINE__,NULL);

  childpid = cfrd_startup(pipin,pipout,"slik_main",NULL,err);
  forwardError(*err,__LINE__,NULL);
