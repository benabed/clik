...
void fill_offset_freq_TP(int idreq,double *dreq, int nfreq, double* freqlist,int *mv,int off, error **err) {
  ...
  for(m=0;m<nfreq;m++) {
    ...
    //_DEBUGHERE_("%d %d %d",m,nfreq,mv[m]);
...
void cnoise_freepow_compute(parametric *egl, double *Rq, error **err);

parametric *cnoise_freepow_init(int ndet_T, int ndet_P, int *has_TEB, double *detlist, int ndef, char** defkey, char **defvalue, int nvar, char **varkey, int lmin, int lmax, error **err) {
  parametric *egl;
  int i,m,*mv,m1,m2;
  double dreq[4];
  pfchar name;
  ...
  egl->eg_compute = &cnoise_freepow_compute;
  egl->eg_free = &parametric_simple_payload_free;

  egl->payload = malloc_err(sizeof(double)* (12*12)*2 + sizeof(int)*egl->nfreq*4,err);
  forwardError(*err,__LINE__,NULL);

  mv = egl->payload + sizeof(double)* ( 12*12)*2;
  ...
  for(m1=0;m1<4;m1++) {
    for(m2=m1;m2<4;m2++) {
      for(f1=0;f1<3;f1++) {
        for(f2=f1;f2<3;f2++) {
          sprintf(name,"A_cnoise_freepow_%d_%d_%c%c",(int)dreq[m1],(int)dreq[m2],teb[f1],teb[f2]);
          parametric_set_default(egl,name,0,err);
          forwardError(*err,__LINE__,NULL);      
        }
      }
  ...
  for(m1=0;m1<4;m1++) {
    for(m2=m1;m2<4;m2++) {
      for(f1=0;f1<3;f1++) {
        for(f2=f1;f2<3;f2++) {
          sprintf(name,"index_cnoise_freepow_%d_%d_%c%c",(int)dreq[m1],(int)dreq[m2],teb[f1],teb[f2]);
          parametric_set_default(egl,name,0,err);
          forwardError(*err,__LINE__,NULL);      
        }
      }
  ...
  parametric_set_default(egl,"cnoise_freepow_l_pivot",2000,err); 
  forwardError(*err,__LINE__,NULL);
...
void cnoise_freepow_compute(parametric *egl, double *Rq, error **err) {
  ...
  mv = egl->payload + sizeof(double)* (12*12)*2;
  A = egl->payload;
  index = egl->payload + sizeof(double)* (12*12);

  l_pivot = parametric_get_value(egl,"cnoise_freepow_l_pivot",err);
  forwardError(*err,__LINE__,);;
    ...
    for(m2=m1;m2<4;m2++) {
      for(f1=0;f1<3;f1++) {
        for(f2=f1;f2<3;f2++) {
          ...
          sprintf(name,"A_cnoise_freepow_%d_%d_%c%c",(int)dreq[m1],(int)dreq[m2],teb[f1],teb[f2]);
          v = parametric_get_value(egl,name,err);
          //_DEBUGHERE_("%s %g",name,v);
          forwardError(*err,__LINE__,);
          rm1 = m1 + f1*4;
          rm2 = m2 + f2*4;
          A[rm1*12+rm2] = (v)/l_pivot/(l_pivot+1)*2*M_PI;
          A[rm2*12+rm1] = A[rm1*12+rm2];
          //_DEBUGHERE_("%s %g %g %d %d %g",name,v,(1-abso+abso*template[((int) l_pivot)*12*12+rm1*12+rm2]/l_pivot/(l_pivot+1)*2*M_PI),rm1,rm2,A[rm1*12+rm2]);
          rm1 = m1 + f2*4;
          rm2 = m2 + f1*4;
          //_DEBUGHERE_("%d",rm1*12+rm2);
          A[rm1*12+rm2] = (v)/l_pivot/(l_pivot+1)*2*M_PI;
          A[rm2*12+rm1] = A[rm1*12+rm2];
          //_DEBUGHERE_("%s %g %g %d %d %g",name,v,(1-abso+abso*template[((int) l_pivot)*12*12+rm1*12+rm2]/l_pivot/(l_pivot+1)*2*M_PI),rm1,rm2,A[rm1*12+rm2]);
    ...
    for(m2=m1;m2<4;m2++) {
      for(f1=0;f1<3;f1++) {
        for(f2=f1;f2<3;f2++) {
          ...
          rm2 = m2 + f2*4;
          index[rm1*12+rm2] = (v);
          index[rm2*12+rm1] = index[rm1*12+rm2];
          //_DEBUGHERE_("%s %g %g %d %d %g",name,v,(1-abso+abso*template[((int) l_pivot)*12*12+rm1*12+rm2]/l_pivot/(l_pivot+1)*2*M_PI),rm1,rm2,A[rm1*12+rm2]);
          rm1 = m1 + f2*4;
          rm2 = m2 + f1*4;
          //_DEBUGHERE_("%d",rm1*12+rm2);
    ...
    for(m1=0;m1<egl->nfreq;m1++) {
      ...
      for(m2=m1;m2<egl->nfreq;m2++) {
        ...
        //if(ell==egl->lmin) {
          //_DEBUGHERE_("%d %d %d %g",ell,mv[m1],mv[m2],template[ell*12*12+mv[m1]*12+mv[m2]]);  
        //}
        //_DEBUGHERE_("%d %d %d %d %d %d %d %d %d",ell,m1,m2,IDX_R(egl,ell,m1,m2),mv[m1],mv[m2],egl->nfreq,egl->nfreq_T,egl->nfreq_P);
        Rq[IDX_R(egl,ell,m1,m2)] = A[mv[m1]*12+mv[m2]] * pow((ell*1./l_pivot),index[mv[m1]*12+mv[m2]]);
        Rq[IDX_R(egl,ell,m2,m1)] = Rq[IDX_R(egl,ell,m1,m2)];
...
CREATE_PARAMETRIC_POL_FILE_INIT(cnoise_freepow,cnoise_freepow_init);
